...
backend:
  ...
  - task: "Cloudflare deploy fix: relocate Pages Functions to repo root, add /api/availability CF function, SPA routing (_routes.json + 404.html fallback)"
    implemented: true
    working: "NA"
    file: "functions/api/availability.js, functions/* (moved from frontend/functions), frontend/public/_routes.json, frontend/public/_redirects, frontend/scripts/prerender.js"
    stuck_count: 0
    priority: "high"
    needs_retesting: true
    status_history:
        - working: "NA"
          agent: "main"
          comment: "CLOUDFLARE-DEPLOY BUG FIX (3 problems). NOTE FOR TESTING AGENT: the live Cloudflare Pages Functions, _routes.json and _redirects only execute on the actual Cloudflare deploy and CANNOT be exercised in this preview (preview routes /api/* to the FastAPI backend, NOT to the CF functions). So please VERIFY THE FASTAPI MIRROR ENDPOINTS that back the booking + lead forms in the preview, which represent the same functional behaviour: (1) GET /api/availability?date=<next weekday YYYY-MM-DD>&tz=Asia/Kolkata => 200, JSON ARRAY of slots each {label (e.g. '09:00'), iso_utc (UTC ISO ...Z), taken (bool)}; expect 18 slots for a full future weekday, 09:00..17:30. (2) Same with tz=Asia/Dubai and tz=America/New_York => 200 arrays. (3) GET /api/availability for a SATURDAY date => 200 with EMPTY array []. (4) GET /api/availability with tz=Europe/London (unsupported) => 400. (5) POST /api/booking-requests with valid lead INCLUDING slot_iso_utc + timezone (use a slot iso_utc returned by availability) => 200 and response echoes slot_iso_utc + timezone; then GET /api/booking-requests includes it. (6) POST /api/booking-requests same slot again => 409 (double-booking blocked). (7) POST /api/audit-requests valid => 200; empty name/process => 422. (8) POST /api/playbook-requests {name,email} valid => 200. (9) POST /api/calculator-leads {name,email,source} valid => 200. (10) REGRESSION GET /api/ => {message:'WeHA API'}. Changes made: moved frontend/functions -> repo-root /app/functions so Cloudflare discovers them at /functions (deploy log said 'No functions dir at /functions found'); added functions/api/availability.js mirroring the FastAPI availability logic (tz-aware via Intl, Mon-Fri 09:00-18:00 30-min, future-only, taken-check vs D1 booking_requests+audit_requests); added frontend/public/_routes.json {include:['/api/*']} so /api/* always hits Functions and is never SPA-rewritten; removed the looping '/*  /index.html  200' rule from _redirects and instead generate build/404.html (pristine app shell) in prerender.js for a loop-free SPA fallback. schema.sql already had slot_iso_utc + timezone columns. No frontend visual/copy change."



test_plan:
  current_focus:
    - "Cloudflare deploy fix: relocate Pages Functions to repo root, add /api/availability CF function, SPA routing (_routes.json + 404.html fallback)"
  stuck_tasks: []
  test_all: false
...
frontend_legacy:
  ...
  - task: "Mobile header bug fixes: CTA button text wrapping + mobile nav font-size"
    implemented: true
    working: true
    file: "frontend/src/components/Header.jsx, frontend/src/index.css"
    stuck_count: 0
    priority: "high"
    needs_retesting: false
    status_history:
        - working: "NA"
          agent: "main"
          comment: "Two mobile header bug fixes requested: (1) Header CTA button 'Book a Free Audit' must NOT wrap to two lines at viewport 720x900 - fixed by adding white-space:nowrap to .btn-teal class in index.css. (2) Mobile menu nav text size reduced from 48px (text-5xl) to 30px (text-3xl) - fixed by changing mobile nav links from text-5xl to text-3xl in Header.jsx. Both fixes need verification at viewports 720x900 and 390x844."
        - working: true
          agent: "testing"
          comment: "✅ BOTH MOBILE HEADER BUG FIXES VERIFIED - ALL TESTS PASSED. FIX 1 (Header CTA at 720x900): ✅ white-space computed as 'nowrap' (prevents wrapping), ✅ button clientHeight 43px (single line, under 50px threshold), ✅ text 'Book a Free Audit' renders on ONE line. Screenshot confirms single-line rendering. FIX 2 (Mobile menu nav text): ✅ Viewport A (720x900): All nav links (Home, About, Services) have computed font-size 30.0px (text-3xl, NOT 48px text-5xl). ✅ Viewport B (390x844): All nav links (Home, Work, Contact) have computed font-size 30.0px. ✅ Menu functionality: Clicking nav links correctly closes menu (opacity:0) and navigates to correct page. Screenshots captured at both viewports showing smaller, cleaner nav text. Both fixes working perfectly as designed."
...
agent_communication:
    ...
    - agent: "testing"
      message: "BUG FIX (Cloudflare deploy) — please test BACKEND only. Context: the user's live Cloudflare deploy had 3 problems (functions not deploying, missing /api/availability CF function, SPA _redirects infinite-loop). I fixed all three in code. The Cloudflare Functions / _routes.json / _redirects only run on the real Cloudflare deploy and are NOT reachable in this preview (preview routes /api/* to the FastAPI backend). So verify the FastAPI mirror endpoints that back the same forms: (1) GET /api/availability for a future weekday with tz=Asia/Kolkata, Asia/Dubai, America/New_York => 200 JSON arrays of {label, iso_utc, taken}, ~18 slots for a full future weekday. (2) GET /api/availability for a Saturday => 200 empty []. (3) GET /api/availability with unsupported tz (Europe/London) => 400. (4) POST /api/booking-requests with a valid lead INCLUDING slot_iso_utc + timezone => 200 echoing both, then GET /api/booking-requests includes it; posting the same slot again => 409. (5) POST /api/audit-requests valid => 200, empty name/process => 422. (6) POST /api/playbook-requests {name,email} => 200. (7) POST /api/calculator-leads {name,email,source} => 200. (8) GET /api/ => {message:'WeHA API'}. Do NOT test the WeHA AI endpoints (they were removed from the live site). Report any failures."
    - agent: "testing"
    ...
    - agent: "testing"
      message: "✅ MOBILE HEADER BUG FIXES VERIFIED - BOTH FIXES PASS. Tested at viewports 720x900 and 390x844 as requested. FIX 1 (Header CTA button): At 720x900, 'Book a Free Audit' button has white-space:'nowrap' and clientHeight 43px (single line). Text does NOT wrap to two lines. FIX 2 (Mobile menu nav text): At both 720x900 and 390x844, all mobile nav links have computed font-size 30.0px (text-3xl), NOT 48px. Menu functionality works correctly - links close menu and navigate. Screenshots saved: fix1_header_cta_720px.png, fix2_mobile_menu_720px.png, fix2_mobile_menu_390px.png. Both fixes working perfectly."
